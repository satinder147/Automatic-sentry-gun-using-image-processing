...
ser=serial.Serial('COM9',9600,timeout=1)                            #MAKING A SERIAL VARIABLE
...
cap=cv2.VideoCapture(1)                                             #CREATING A CAP VARIABLE THAT HOLDS THE FRAME FROM THE CAMERA
while(1):                                                           #STARTING INFINITE LOOP FOR CAPTURING VIDEOS
    ...
    cv2.line(frame,(int(frame.shape[1]/2),0),(int(frame.shape[1]/2),int(frame.shape[0])),(0,255,0),2)
                                                                  #FRAME VARIABLE READS THE VIDEOS STREAM FRAME BY FRAME FROM THE WEBCAM                      
    frame2=frame
    ...
    if(len(contours)>0):                                                  #IF RED WAS DETECTED
        for i in range(len(contours)):                                    #LOOPING BETWEEN ALL THE CONTOURS
            ...
            if(m['m00']!=0):                                              
                ...
                #b=int(m['m01']/m['m00']
                print('')#THIS GIVES THE CENTER(X COORDINATE) OF THE RED SPOT                         
     
    circles=cv2.HoughCircles(gray,cv2.HOUGH_GRADIENT,1.0,100)             #HOUGH CIRCLE FINDS THE CIRCLES IqN THE VIDEO
